...
TEST(Commands, test_begin) {
    ...
    beg.configure("", 15, stack_);
    EXPECT_EQ(beg.run("", 15), 16);
    EXPECT_EQ(beg.get_line(), 15);

    BeginCommand beg_incorrect = BeginCommand();
    EXPECT_THROW(beg_incorrect.configure("argument", 0, stack_), InvalidArgumentException);

    EXPECT_THROW(beg.configure("", 5, stack_), UniqueException);
}

TEST(Commands, test_end) {
    EndCommand end = EndCommand();
    end.configure("", 14, stack_);
    EXPECT_EQ(end.run("", 14), -1);

    EndCommand end_incorrect = EndCommand();
    EXPECT_THROW(end_incorrect.configure("argument", 4, stack_), InvalidArgumentException);

    EXPECT_THROW(end.configure("", 5, stack_), UniqueException);
}

TEST(Commands, test_push) {
    PushCommand push = PushCommand();
    push.configure("42", 2, stack_);
    EXPECT_EQ(push.run("42", 2), 3);
    EXPECT_EQ(stack_->data.top(), 42);

    PushCommand push_incorrect = PushCommand();
    EXPECT_THROW(push_incorrect.configure("ax", 4, stack_), std::invalid_argument);
    EXPECT_THROW(push_incorrect.configure("9999999999999999", 4, stack_), InvalidArgumentException);
}
...
TEST(Commands, test_pop) {
    ...
    pop.configure("", 2, stack_);
    EXPECT_EQ(pop.run("", 2), 3);
    EXPECT_EQ(size, stack_->data.size());

    PopCommand pop_incorrect = PopCommand();
    EXPECT_THROW(pop_incorrect.configure("o", 4, stack_), InvalidArgumentException);
}

TEST(Commands, test_pushr) {
    RegisterType::get("ax").value() = -777;
    PushRCommand pushr = PushRCommand();
    pushr.configure("ax", 2, stack_);
    EXPECT_EQ(pushr.run("ax", 2), 3);
    EXPECT_EQ(stack_->data.top(), -777);

    PushRCommand pushr_incorrect = PushRCommand();
    EXPECT_THROW(pushr_incorrect.configure("", 4, stack_), InvalidArgumentException);
}
...
TEST(Commands, test_popr) {
    ...
    popr.configure("cx", 2, stack_);
    EXPECT_EQ(popr.run("cx", 2), 3);
    EXPECT_EQ(RegisterType::get("cx").value(), 993);
    EXPECT_EQ(size, stack_->data.size());

    PopRCommand popr_incorrect = PopRCommand();
    EXPECT_THROW(popr_incorrect.configure("o", 4, stack_), InvalidArgumentException);
}
...
TEST(Commands, test_add) {
    ...
    add.configure("", 777, stack_);
    EXPECT_EQ(add.run("", 777), 778);
    EXPECT_EQ(1000, stack_->data.top());

    AddCommand add_incorrect = AddCommand();
    EXPECT_THROW(add_incorrect.configure("38", 4, stack_), InvalidArgumentException);
}
...
TEST(Commands, test_sub) {
    ...
    sub.configure("", 0, stack_);
    EXPECT_EQ(sub.run("", 0), 1);
    EXPECT_EQ(993, stack_->data.top());

    SubCommand sub_incorrect = SubCommand();
    EXPECT_THROW(sub_incorrect.configure("string", 4, stack_), InvalidArgumentException);
}
...
TEST(Commands, test_mul) {
    ...
    mul.configure("", 5, stack_);
    EXPECT_EQ(mul.run("", 5), 6);
    EXPECT_EQ(-7982 * 33, stack_->data.top());

    MulCommand mul_incorrect = MulCommand();
    EXPECT_THROW(mul_incorrect.configure("AHAHAAHAH", 4, stack_), InvalidArgumentException);
}
...
TEST(Commands, test_div) {
    ...
    div.configure("", 5, stack_);
    EXPECT_EQ(div.run("", 5), 6);
    EXPECT_EQ(-5, stack_->data.top());

    DivCommand div_incorrect = DivCommand();
    EXPECT_THROW(div_incorrect.configure("100^Pi", 4, stack_), InvalidArgumentException);
}
...
TEST(Commands, test_in) {
    ...
    in.configure("", 5, stack_);
    EXPECT_EQ(in.run("", 5), 6);
    EXPECT_EQ(1524, stack_->data.top());
    EXPECT_EQ("Input number: ", s_out.str());

    InCommand in_incorrect = InCommand();
    EXPECT_THROW(in_incorrect.configure("rax", 1, stack_), InvalidArgumentException);
...
TEST(Commands, test_out) {
    ...
    out.configure("", 5, stack_);
    EXPECT_EQ(out.run("", 5), 6);
    EXPECT_EQ(s.str(), "8888888\n");
    EXPECT_EQ(7777777, stack_->data.top());

    OutCommand out_incorrect = OutCommand();
    EXPECT_THROW(out_incorrect.configure("eax", 1, stack_), InvalidArgumentException);
...
TEST(Commands, test_label) {
    ...
    label.configure("sett0ings", 5, stack_);
    EXPECT_EQ(label.run("sett0ings", 5), 6);
    ...
    EXPECT_THROW(label_incorrect.configure("1_name", 1, stack_), InvalidArgumentException);
    EXPECT_THROW(label_incorrect.configure("name_", 1, stack_), InvalidArgumentException);
}

TEST(Commands, test_jump) {
    LabelCommand label = LabelCommand();
    label.configure("target", 5, stack_);
    label.run("target", 5);

    JumpCommand jump = JumpCommand();
    jump.configure("target", 15, stack_);
    EXPECT_EQ(jump.run("target", 15), 5);

    JumpCommand jump_incorrect = JumpCommand();
    jump_incorrect.configure("target2", 1, stack_);
    EXPECT_THROW(jump_incorrect.run("target2", 1), InvalidArgumentException);
}
...
TEST(Commands, test_jump_equal) {
    ...
    EXPECT_EQ(jump.run("target", 15), 16);
...
TEST(Commands, test_call) {
    ...
    call.configure("target", 15, stack_);
    EXPECT_EQ(call.run("target", 15), 5);
...
TEST(Commands, test_ret) {
    ...
    ret.configure("", 5, stack_);
    EXPECT_EQ(ret.run("", 5), 16);
    EXPECT_TRUE(stack_->call.empty());
    EXPECT_THROW(ret.run("target", 5), std::runtime_error);

    RetCommand ret_incorrect = RetCommand();
    EXPECT_THROW(ret_incorrect.configure("target", 1, stack_), InvalidArgumentException);
}